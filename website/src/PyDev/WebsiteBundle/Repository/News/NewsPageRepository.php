...
class NewsPageRepository extends AbstractArticlePageRepository
{
    ...
    public function getArticlesQuery($lang = null, $offset, $limit)
    {
        ...
        $rsm->addRootEntityFromClassMetadata('PyDev\WebsiteBundle\Entity\News\NewsPage', 'qp');

        $query = "SELECT";
        $query .= " article.*";
        $query .= " FROM";
        $query .= " pydev_websitebundle_news_pages as article";
        $query .= " INNER JOIN";
        ...
        $query .= " n.deleted = 0";
        $query .= " AND";
        $query .= " n.ref_entity_name = 'PyDev\\\\WebsiteBundle\\\\Entity\\\\News\\\\NewsPage'";
        $query .= " AND";
        $query .= " nt.online = 1 ";
        if ($lang) {
            $query .= " AND";
